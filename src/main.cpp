...
#if defined(NDEBUG)
#error "EREX cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight >= 50 && nHeight < 50000) {
    	return 1 * COIN;
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
	...
	uiInterface.ThreadSafeMessageBox(_("You are on the wrong chain.  Please check debug.log for more information on resolving this.  Shutting down."), "", CClientUIInterface::MSG_ERROR);
        LogPrintf("Your block database is from a forked version of the correct chain.  Please visit https://github.com/EREXCoin/bootstrap to avoid any potential loss of future rewards and get on the proper chain.\n");
        StartShutdown();
        return false;
      }
    }
